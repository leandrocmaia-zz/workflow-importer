...
public class WorkflowInstanceMigrator extends BaseMigrator implements Migrator {
  ...
  @Getter List<WorkflowInstance> workflowInstances = new ArrayList<>();

  /** entries that failed to parse, attributes not found or no lookup was found. */
  @Getter List<WorkflowInstance> workflowInstancesWithError = new ArrayList<>();

  /** lines outside start/end loop */
  @Getter List<String> unparsableLines = new ArrayList<>();
  ...
  /**
   * There is strong dependency from other migrators in this one. Using dependency injection via
   * constructor.
   *
   * @param file
   ...
   */
  public WorkflowInstanceMigrator(
      File file,
                ...
                workflowMigrator
                    ...
                    .orElseGet(
                        () -> {
                          ...
                          return new Workflow(
                              new Long(value)); // in case of being persistable later
                        });
   ...
   *
   * <p>• Showing any inconsistent entries. • Showing all workflows with their according instances.
   * • Showing all workflows having running instances and the number of those instances. • Showing
   * all contractors that are assignees to running instances
   */
  @Override
  public void logSummary() {
    ...
    log.info(
        "Workflows with running instances {}: {}",
        running.size() + "/" + getWorkflowInstances().size(),
        gson.toJson(running));

    log.info(
        "All contractors who are assigned to running instances: {}",
        gson.toJson(
            running
                .stream()
